...
bool GameScene10::init()
{
	...
	girl->setPosition(Point(size.width,0));
	...
	OpenLamp->setTag(4);
	OpenLamp->setPosition(Point(size.width*0.40,size.height*0.31));
	...
	Draw->setPosition(Point(size.width*0.13,size.height*0.75));
	Draw->setTag(5);
	...
	DrawWithKey->setPosition(Point(size.width*0.13,size.height*0.75));
	DrawWithKey->setTag(6);
	DrawWithKey->setVisible(false);
	 this->addChild(DrawWithKey,1);

	 //hole
	 auto hole=Sprite::create("level10/10-hole.png");
	 hole->setPosition(Point(size.width*0.75,size.height*0.75));
	 hole->setTag(13);
	 this->addChild(hole,3);
	...
	//Draw' key 
	auto key2=Sprite::create("level10/10-key.png");
    key2->setPosition(Point(size.width*0.23,size.height*0.79));
	key2->setVisible(false);
	key2->setTag(9);
	this->addChild(key2,3);


	//question
	auto question=Sprite::create("level10/10-question.png");
    question->setPosition(Point(size.width*0.70,size.height*0.55));
...
void GameScene10::onTouchMovedDraw(Touch *touch, Event* event){
    ...
    //定制一个框框  
    auto rect = Rect(keyPos.x - keySize.width/2,keyPos.y - keySize.height/2,keySize.width + 20,keySize.height + 10);  
...
void GameScene10::onTouchEndedDraw(Touch *touch, Event* event){
    ...
    //定制一个框框  
    auto rect = Rect(DrawPos.x - DrawSize.width/2,DrawPos.y - DrawSize.height/2,DrawSize.width + 10,DrawSize.height + 10);  //建立一个选中区域，女主如果拖动到这个框内就可以瞬移进来，反之是小三进来  
   ...
   if(DrawIsWithKey==1){
    ...
    if(rect.containsPoint(lastPos)) {  
		auto size = Director::getInstance()->getWinSize();  
        moveTo_2 = MoveTo::create(0.1f,Point(size.width*0.15,size.height*0.10));  
        this->getChildByTag(1)->runAction(moveTo_2);
   }
}
...
void  GameScene10::onTouchMovedBox(Touch *touch, Event* event){
    ...
    //定制一个框框  
    auto rect = Rect(key2Pos.x - key2Size.width/2,key2Pos.y - key2Size.height/2,key2Size.width + 20,key2Size.height + 10);  
    if(rect.containsPoint(beginPos))//判断触点是否在身上！  
    {  
        auto endPos = touch->getPreviousLocationInView();//获取触摸的前一个位置  
        endPos = Director::sharedDirector()->convertToGL(endPos);//转换坐标  
...
void  GameScene10::onTouchEndedBox(Touch *touch, Event* event){
    ...
    //定制一个框框  
    auto rect = Rect(BoxPos.x - BoxSize.width/2,BoxPos.y - BoxSize.height/2,BoxSize.width + 10,BoxSize.height + 10);  //建立一个选中区域，女主如果拖动到这个框内就可以瞬移进来，反之是小三进来  
  ...
  if(OpenBox==1){
    if(rect.containsPoint(lastPos))   {  
		...
		auto size = Director::getInstance()->getWinSize();  
        moveTo_3 = MoveTo::create(0.1f,Point(size.width*0.23,size.height*0.79));  
        this->getChildByTag(9)->runAction(moveTo_3);
 }  
  }
...
void GameScene10::onTouchEndedHole(Touch *touch, Event* event){
    ...
    //定制一个框框  
    auto rect = Rect(holePos.x - holeSize.width/2,holePos.y - holeSize.height/2,holeSize.width + 10,holeSize.height + 10);  //建立一个选中区域，女主如果拖动到这个框内就可以瞬移进来，反之是小三进来  
    ...
    if(rect.containsPoint(lastPos))   {  
        ...
        this->getChildByTag(14)->runAction(RotateTo::create(1.0f,0));
	 }  else  {  
		auto size = Director::getInstance()->getWinSize();  
